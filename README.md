...
2. This approach, smartly used, allows to build extraction code that can be progressively more general, so well designed code is useful for many different sites, and it is independent of html layout.
   Html code is extremely variable. But how information is presented to a human, it is not. Instead you can find many patterns.
3. As corollary, it allow the generation of datasets suitable for training transformer models for text extraction (aka extractive question answering, EQA). The dataset records are generated through the usage of
  ...
  `add_xpath()` operate on the html domain. So in the second case, extraction of data for dataset generation is computed by searching the extracted value from html, not the markdown. Because of this,
...
4. As a secondary objective, it provides an alternative kind of selector to xpath and css selectors for extraction of data from the html source, that may be more suitable and readable for humans.
5. In many situations, and specially when there is not an id or a class to spot accurately the text, the expresion in terms of regular expressions in the domain of markdown can be simpler.
6. The additional cost of maintenance is zero. Even if you are introducing a new kind of selector that may be unknown for most people, this only characterizes single lines and does not condition
   ...
   anything else. However, as said, the power of the method resides in generalization, that is, not site specific code, but generic one that works for any site of same category of sites.
